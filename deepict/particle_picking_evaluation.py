...
# Dictionary of particle radii in voxels (Å / voxel_size) based on competition data
# The voxel size is assumed to be 10Å
PARTICLE_RADII = {
    'apo-ferritin': 60,
    'beta-amylase': 65,
...
# Dictionary for particle weights as specified in the competition (for weighted F-beta)
PARTICLE_WEIGHTS = {
    'apo-ferritin': 1,       # easy
    'ribosome': 1,           # easy
    'virus-like-particle': 1, # easy
    'beta-galactosidase': 2,  # hard
    'thyroglobulin': 2        # hard
}

# Competition F-beta parameter
BETA = 4.0
...
if run_job:
    ...
    # Use the particle type for evaluation - fallback to config.pred_class if undefined
    particle_type = config.pred_class
    
    # Now using the particle radii for evaluation
    print(f"Evaluating {particle_type} with competition criteria (0.5 × radius)")
    ...
    # Calculate competition F-beta score (beta=4)
    ...
    print(f"Competition metric: max_F_beta(beta={BETA}) = {max_F_beta}; final F_beta = {F_beta_score[-1]}")
    ...
    # Add F-beta metrics to statistics
    write_statistics_pp(statistics_file=statistics_file, tomo_name=tomo_name, model_descriptor=model_descriptor,
                        statistic_variable=f"max_F_beta_{BETA}",
                        statistic_value=round(max_F_beta, 4), pr_radius=config.pr_tolerance_radius,
                        min_cluster_size=config.min_cluster_size, max_cluster_size=config.max_cluster_size,
                        threshold=config.threshold, prediction_class=config.pred_class,
                        clustering_connectivity=config.clustering_connectivity, processing_tomo=config.processing_tomo,
                        region_mask=config.region_mask)
    ...
    write_statistics_pp(statistics_file=statistics_file, tomo_name=tomo_name, model_descriptor=model_descriptor,
                        statistic_variable=f"F_beta_{BETA}",
                        statistic_value=round(F_beta_score[-1], 4), pr_radius=config.pr_tolerance_radius,
                        min_cluster_size=config.min_cluster_size, max_cluster_size=config.max_cluster_size,
                        threshold=config.threshold, prediction_class=config.pred_class,
                        clustering_connectivity=config.clustering_connectivity, processing_tomo=config.processing_tomo,
                        region_mask=config.region_mask)
